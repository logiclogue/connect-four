...
{
    return
        is_horizontal_line(self) ||
        is_vertical_line(self) ||
        is_positive_diagonal_line(self) ||
        is_negative_diagonal_line(self);
}
...
static int check_line(LineChecker *self, int delta_column, int delta_row)
{
    ...
    int max_column = get_maximum(self, self->columns, delta_column);
    int max_row = get_maximum(self, self->rows, delta_row);
    int min_column = get_minimum(self, delta_column);
    int min_row = get_minimum(self, delta_row);
    char piece, current_piece;

    for (column = min_column; column < self->columns; column += 1) {
        for (row = min_row; row < self->rows; row += 1) {
            piece = self->grid->get(column, row);

            for (i = 0, i < self->length; i += 1) {
                testing_column = column + (delta_column * i);
                testing_row = column + (delta_row * i);
                current_piece = self->grid->get(testing_column, testing_row)
